...
export default function SubmissionForm({ categories }: Props) {
  ...
  const heroMediaIdRef = useRef<number | null>(null)
  ...
  const uploadHeroImage = useCallback(async (file: File) => {
    ...
    try {
      const fd = new FormData()
      fd.append('file', file)
      // Pass the previous media ID so the upload route can delete the orphaned file
      if (heroMediaIdRef.current) fd.append('replacesMediaId', String(heroMediaIdRef.current))
      const res = await fetch('/api/media/upload', { method: 'POST', body: fd })
      const json = (await res.json()) as { url?: string; id?: number; error?: string }
      if (!res.ok || !json.url) throw new Error(json.error ?? 'Upload failed')
      setHeroMediaId(json.id ?? null)
      heroMediaIdRef.current = json.id ?? null
      setFieldErrors((prev) => ({ ...prev, heroImage: undefined }))
      logger.info('Hero image uploaded', { url: json.url, id: json.id })
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Upload failed'
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor: e }) => {
      if (fieldErrors.content && e.getText().trim())
  ...
  const validateEmail = (val: string): string | undefined => {
    ...
    if (parts.length !== 2 || !parts[1].includes('.')) return 'Please enter a valid email address.'
    return undefined
  }

  const validateUrl = (val: string): string | undefined => {
    if (!val.trim()) return 'A website or LinkedIn URL is required.'
    try {
      const u = new URL(val.trim())
      if (u.protocol !== 'http:' && u.protocol !== 'https:') return 'URL must start with https://'
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Client-side field validation before hitting the server
    const errors: typeof fieldErrors = {}
    if (!title.trim()) errors.title = 'A title is required.'
    const editorText = editor?.getText().trim()
    if (!editorText) errors.content = 'Article body is required.'
    if (!authorName.trim()) errors.authorName = 'Your name is required.'
    const emailErr = validateEmail(authorEmail)
    if (emailErr) errors.email = emailErr
    const urlErr = validateUrl(authorUrl)
    if (urlErr) errors.authorUrl = urlErr
    if (!category) errors.category = 'Please select a section.'
    if (!heroMediaId) errors.heroImage = 'Please upload a cover image.'
  ...
  if (submitted) {
    return (
      <div role="status" aria-live="polite" aria-atomic="true" className="write-success">
        <svg
          className="write-success-icon"
          viewBox="0 0 48 48"
          fill="none"
          stroke="currentColor"
          strokeWidth="1"
        ...
        <p>
          Our editorial team will review your submission and be in touch within 3–5 business days.
        </p>
    ...
    <form className="write-form" onSubmit={handleSubmit}>
        ...
        <div className="write-meta-grid">
          <div className="field">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (fieldErrors.authorName && e.target.value.trim())
                  setFieldErrors((prev) => ({ ...prev, authorName: undefined }))
              }}
              onBlur={(e) => {
                if (!e.target.value.trim())
                  setFieldErrors((prev) => ({ ...prev, authorName: 'Your name is required.' }))
              }}
              placeholder="Full name"
              style={fieldErrors.authorName ? { borderColor: '#DC2626' } : undefined}
              required
            />
          ...
          <div className="field field--full">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (fieldErrors.authorUrl)
                  setFieldErrors((prev) => ({ ...prev, authorUrl: validateUrl(e.target.value) }))
              }}
              onBlur={(e) =>
                setFieldErrors((prev) => ({ ...prev, authorUrl: validateUrl(e.target.value) }))
              }
              placeholder="https://linkedin.com/in/yourname or https://yoursite.com"
              style={fieldErrors.authorUrl ? { borderColor: '#DC2626' } : undefined}
              required
            />
            {fieldErrors.authorUrl && (
              <p style={{ margin: '4px 0 0', fontSize: 12, color: '#DC2626' }}>
                {fieldErrors.authorUrl}
              </p>
            )}
          ...
          <div className="field">
            ...
            <select
              ...
              onBlur={(e) => {
                if (!e.target.value)
                  setFieldErrors((prev) => ({ ...prev, category: 'Please select a section.' }))
              }}
              style={fieldErrors.category ? { borderColor: '#DC2626' } : undefined}
              required
            >
            ...
            {heroImagePreview ? (
              ...
              <button
                ...
                style={{
                  width: '100%',
                  marginTop: 4,
                  border: `1px dashed ${fieldErrors.heroImage ? '#DC2626' : '#cfc4c5'}`,
                  borderRadius: 3,
                ...
                onMouseOut={(e) => {
                  e.currentTarget.style.borderColor = '#cfc4c5'
      ...
      <div className="write-area">
        <textarea
          ...
          onBlur={(e) => {
            if (!e.target.value.trim())
              setFieldErrors((prev) => ({ ...prev, title: 'A title is required.' }))
          }}
          ref={autoResize}
          style={fieldErrors.title ? { borderBottom: '1px solid #DC2626' } : undefined}
        />
        {fieldErrors.title && (
          <p style={{ margin: '4px 0 0', fontSize: 12, color: '#DC2626' }}>{fieldErrors.title}</p>
        )}
        <textarea
        ...
        {editor && (
          <BubbleMenu editor={editor} tippyOptions={{ duration: 80, placement: 'top' }}>
            <div className="write-bubble">
              <button
                type="button"
                className={`write-bubble-btn${editor.isActive('bold') ? ' is-active' : ''}`}
                onMouseDown={(e) => {
                  e.preventDefault()
              ...
              <button
                type="button"
                className={`write-bubble-btn${editor.isActive('italic') ? ' is-active' : ''}`}
                onMouseDown={(e) => {
                  e.preventDefault()
              ...
              <button
                type="button"
                className={`write-bubble-btn${editor.isActive('heading', { level: 2 }) ? ' is-active' : ''}`}
                onMouseDown={(e) => {
                  e.preventDefault()
              ...
              <button
                type="button"
                className={`write-bubble-btn${editor.isActive('heading', { level: 3 }) ? ' is-active' : ''}`}
                onMouseDown={(e) => {
                  e.preventDefault()
              ...
              <button
                type="button"
                className={`write-bubble-btn${editor.isActive('blockquote') ? ' is-active' : ''}`}
                onMouseDown={(e) => {
                  e.preventDefault()
                  editor.chain().focus().toggleBlockquote().run()
                }}
              ...
              <button
                type="button"
                className={`write-bubble-btn${editor.isActive('bulletList') ? ' is-active' : ''}`}
                onMouseDown={(e) => {
                  e.preventDefault()
        ...
        {editor && (
          <FloatingMenu
            editor={editor}
            tippyOptions={{ duration: 80, placement: 'left', offset: [-4, 16] }}
          >
              ...
              {menuOpen && activePanel === 'image' && (
                <div className="float-image-sub">
                  ...
                  <div className="float-url-row">
                    ...
                    />
                    <button type="button" className="float-url-go" onClick={insertImageByUrl}>
                      ↵
                    </button>
                    <button type="button" className="float-url-close" onClick={closeMenu}>
                      ✕
                    </button>
                  </div>
                </div>
              ...
              {menuOpen && activePanel === 'video' && (
                <div className="float-image-sub">
                  <div className="float-url-row float-url-row--wide">
                    ...
                    />
                    <button type="button" className="float-url-go" onClick={insertVideo}>
                      ↵
                    </button>
                    <button type="button" className="float-url-close" onClick={closeMenu}>
                      ✕
        ...
        <EditorContent
          ...
          onBlur={() => {
            if (!editor?.getText().trim())
              setFieldErrors((prev) => ({ ...prev, content: 'Article body is required.' }))
            else setFieldErrors((prev) => ({ ...prev, content: undefined }))
          }}
        />
        {fieldErrors.content && (
          <p style={{ margin: '4px 0 0', fontSize: 12, color: '#DC2626' }}>{fieldErrors.content}</p>
        )}
      </div>
